...
You should have some experience running `R` code, it is a prerequisite of the course.
But before we move forward it is important your remember that frustration as you ler to use `R` is natural when you start programming in R because it is such a stickler for punctuation, and even one character out of place can cause it to complain.
...
A good code formatting practice as you make assignments is to surrounds `<-` with spaces.
...
We'll return to names again when we discuss code style.
...
knitr::include_graphics("rstudio-env.png")
```
