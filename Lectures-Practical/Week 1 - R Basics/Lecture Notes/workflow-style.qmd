...
This lesson will introduce the most important points of the [tidyverse style guide](https://style.tidyverse.org), which is used throughout this book.
...
knitr::include_graphics("rstudio-palette.png")
```
...
We talked briefly about names in the **Workflow: basics** lesson.
Remember that variable names (those created by `<-`) should use only lowercase letters, numbers, and `_`.
...
For example, if you're creating multiple variables using `mutate()`, you might want to add spaces so that all the `=` line up.[^1]
...
knitr::include_graphics("rstudio-nav.png")
```
