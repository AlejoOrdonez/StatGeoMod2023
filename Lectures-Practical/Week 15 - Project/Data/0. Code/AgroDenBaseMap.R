...
## Agricultual Density:  Agricultual density is calculated as the proportion of 
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Agricultual area Density based on Basemap
...
ReclassMrtx <- cbind(as.numeric(BaseMapUnique), # These are the "is" values [those in the raster]
                     ifelse(substring(as.vector(BaseMapUnique),1,1)==NatAreaVal,1, # These are the "becomes" values [the NEW values]
...
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Agricultual area 
Basemap.2011.Agr <- reclassify(x = BasemapRaw.2011, # raster to reclasify
                                  rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2011.Agr)

### Now we are ready to aggregate the 10x10m reclassified basemap raster into a 1x1km.
### Here you will determine the density of Agriculture areas per 1km [km of Agricultual area per km
...
Basemap.Agr1k.2011 <- aggregate(x = Basemap.2011.Agr, # raster to aggregate
...
plot(Basemap.Agr1k.2011)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2016 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Agricultual area 
Basemap.2016.Agr <- reclassify(x = BasemapRaw.2016, # raster to reclasify
                                  rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2016.Agr)

### Now we are ready to aggregate the 10x10m reclassified basemap raster into a 1x1km.
### Here you will determine the density of Agricultural areas per 1km [km of Agricultual area per km

# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.Agr1k.2016 <- aggregate(x = Basemap.2016.Agr, # raster to aggregate
                                   fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                   fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.Agr1k.2016)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2018 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Agricultual area 
Basemap.2018.Agr <- reclassify(x = BasemapRaw.2018, # raster to reclasify
                                  rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2018.Agr)
...
# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.Agr1k.2018 <- aggregate(x = Basemap.2018.Agr, # raster to aggregate
                                   fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                   fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.Agr1k.2018)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Average of Agricultral density in 2012 and 2018
Basemap.Agr1k <- mean(stack(Basemap.Agr1k.2018,Basemap.Agr1k.2016,Basemap.Agr1k.2011))

plot(Basemap.Agr1k)

## Save the Raster
writeRaster(x = Basemap.Agr1k, # raster to save
            filename = "~/Dropbox/Courses_&_Conferences/2022/Courses/Statistical and Geospatial Modeling/Lectures-Practical/Project/Data/Agro Density/AgroDens_Basemap.tif", # Name of the raster
