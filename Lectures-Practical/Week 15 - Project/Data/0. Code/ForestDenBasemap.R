...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Open nature Density based on Basemap

## Define the value for forest areas [There have a value of 3 at level 1]
NatAreaVal <- 31 
...
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Forests 
Basemap.2011.Forest<- reclassify(x = BasemapRaw.2011, # raster to reclasify
                                  rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2011.Forest)

### Now we are ready to aggregate the 10x10m reclassified basemap raster into a 1x1km.
### Here you will determine the density of Open nature areas per 1km [km of Open nature per km
...
Basemap.Forest1k.2011 <- aggregate(x = Basemap.2011.Forest, # raster to aggregate
...
plot(Basemap.Forest1k.2011)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2016 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Forests 
Basemap.2016.Forest<- reclassify(x = BasemapRaw.2016, # raster to reclasify
                                  rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2016.Forest)
...
# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.Forest1k.2016 <- aggregate(x = Basemap.2016.Forest, # raster to aggregate
                                   fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                   fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.Forest1k.2016)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2018 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Forests 
Basemap.2018.Forest<- reclassify(x = BasemapRaw.2018, # raster to reclasify
                                  rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2018.Forest)
...
# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.Forest1k.2018 <- aggregate(x = Basemap.2018.Forest, # raster to aggregate
                                   fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                   fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.Forest1k.2018)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Average of Natural density in 2012 and 2018
Basemap.Forest1k <- mean(stack(Basemap.Forest1k.2018,Basemap.Forest1k.2016,Basemap.Forest1k.2011))

plot(Basemap.Forest1k)

## Save the Raster
writeRaster(x = Basemap.Forest1k, # raster to save
            filename = "~/Dropbox/Courses_&_Conferences/2022/Courses/Statistical and Geospatial Modeling/Lectures-Practical/Project/Data/Forest Density/ForestDens_Basemap.tif", # Name of the raster
