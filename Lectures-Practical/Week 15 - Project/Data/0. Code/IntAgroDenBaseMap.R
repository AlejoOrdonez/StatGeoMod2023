...
## Intensive Agriculture Density:  Intensive Agriculture is calculated as the proportion of 
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Intensive agricultual area Density based on Basemap
...
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Intensive agricultual area 
Basemap.2011.IntAgr <- reclassify(x = BasemapRaw.2011, # raster to reclasify
                               rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2011.IntAgr)

### Now we are ready to aggregate the 10x10m reclassified basemap raster into a 1x1km.
### Here you will determine the density of intensive agricultural areas per 1km [km of Intensive agricultual area per km
...
Basemap.IntAgr1k.2011 <- aggregate(x = Basemap.2011.IntAgr, # raster to aggregate
...
plot(Basemap.IntAgr1k.2011)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2016 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Intensive agricultual area 
Basemap.2016.IntAgr <- reclassify(x = BasemapRaw.2016, # raster to reclasify
                               rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2016.IntAgr)
...
# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.IntAgr1k.2016 <- aggregate(x = Basemap.2016.IntAgr, # raster to aggregate
                                fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.IntAgr1k.2016)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2018 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Intensive agricultual area 
Basemap.2018.IntAgr <- reclassify(x = BasemapRaw.2018, # raster to reclasify
                               rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2018.IntAgr)
...
# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.IntAgr1k.2018 <- aggregate(x = Basemap.2018.IntAgr, # raster to aggregate
                                fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.IntAgr1k.2018)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Average of Intensive Agricultural density in 2012 and 2018
Basemap.IntAgr1k <- mean(stack(Basemap.IntAgr1k.2018,Basemap.IntAgr1k.2016,Basemap.IntAgr1k.2011))

plot(Basemap.IntAgr1k)

## Save the Raster
writeRaster(x = Basemap.IntAgr1k, # raster to save
            filename = "~/Dropbox/Courses_&_Conferences/2022/Courses/Statistical and Geospatial Modeling/Lectures-Practical/Project/Data/4.Agro Density/IntAgroDens_Basemap.tif", # Name of the raster
