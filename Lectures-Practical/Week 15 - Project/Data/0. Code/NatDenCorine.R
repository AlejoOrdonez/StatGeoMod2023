...
### FOREST AND SEMI- NATURAL AREAS include
### 3.1. Forests
...
CorineDK.2012.Nat <- reclassify(x = CorineDK.2012, # raster to reclassify
...
### Reclassify the raster so that it ONLY shows the areas considered FOREST AND SEMI- NATURAL AREAS 
CorineDK.2018.Nat <- reclassify(x = CorineDK.2018, # raster to reclassify
                                rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(CorineDK.2018.Nat)
...
CorineDK.2018.Nat1k <- aggregate(x = CorineDK.2018.Nat, # raster to aggregate
                                 fact = 10, # Aggregation factor (number of cells in each direction to merge together)
                                 fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(CorineDK.2018.Nat1k)

#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Average of Natural density in 2012 and 2018
CorineDK.Nat1k <- mean(stack(CorineDK.2018.Nat1k,CorineDK.2012.Nat1k))

plot(CorineDK.Nat1k)

## Save the Raster
writeRaster(x = CorineDK.Nat1k, # raster to save
            filename = "/Users/alejandroordonez/Dropbox/Courses_&_Conferences/2022/Courses/Statistical and Geospatial Modeling/Lectures-Practical/Project/Data/Nature Density/NatDens_Cor.tif", # Name of the raster
            format="GTiff", overwrite=TRUE) # Arguments
