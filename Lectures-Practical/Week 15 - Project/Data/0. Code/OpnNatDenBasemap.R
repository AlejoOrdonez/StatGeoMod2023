...
## Define the value for Open natural areas [There have a value of 3 at level 1]
NatAreaVal <- 32 
...
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Open nature 
Basemap.2011.OpmNat <- reclassify(x = BasemapRaw.2011, # raster to reclasify
                               rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2011.OpmNat)
...
Basemap.OpmNat1k.2011 <- aggregate(x = Basemap.2011.OpmNat, # raster to aggregate
...
plot(Basemap.OpmNat1k.2011)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2016 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Open nature 
Basemap.2016.OpmNat <- reclassify(x = BasemapRaw.2016, # raster to reclasify
                               rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2016.OpmNat)
...
# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.OpmNat1k.2016 <- aggregate(x = Basemap.2016.OpmNat, # raster to aggregate
                                fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.OpmNat1k.2016)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Basemap 2018 
a<-Sys.time()
### Reclassify the raster so that it ONLY shows the areas considered Open nature 
Basemap.2018.OpmNat <- reclassify(x = BasemapRaw.2018, # raster to reclasify
                               rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(Basemap.2018.OpmNat)
...
# Aggregate the 100x100m dataset
Summ.Func <- function(x, na.rm=TRUE){sum(x, na.rm=TRUE)/sum(!is.na(x))} # Define the density of points of a class [sum(x)] in all land sites [sum(!is.na(x))]
Basemap.OpmNat1k.2018 <- aggregate(x = Basemap.2018.OpmNat, # raster to aggregate
                                fact = 100, # Aggregation factor (number of cells in each direction to merge together)
                                fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclassified map
plot(Basemap.OpmNat1k.2018)
Sys.time()-a
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Average of Open Natural density in 2012 and 2018
Basemap.OpmNat1k <- mean(stack(Basemap.OpmNat1k.2018,Basemap.OpmNat1k.2016,Basemap.OpmNat1k.2011))

plot(Basemap.OpmNat1k)

## Save the Raster
writeRaster(x = Basemap.OpmNat1k, # raster to save
            filename = "~/Dropbox/Courses_&_Conferences/2022/Courses/Statistical and Geospatial Modeling/Lectures-Practical/Project/Data/Open Nature Density/OpnNatDens_Basemap.tif", # Name of the raster
