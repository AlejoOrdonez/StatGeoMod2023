...
### the Second aggregation for AGRICULTURAL AREAS is:
### 2.1. Arable land
### 2.2. Permanent crops
### 2.3. Pastures
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Agricultral Density based on Corine

## Define the value for Open natural areas 
NatAreaVal <- c(23) 

## Build a Reclassification matrix
ReclassMrtx <- cbind(1:dim(Legend)[1],  # These are the "is" values [those in the raster]
                     substring(Legend[,1],1,2)%in%NatAreaVal # These are the "becomes" values [the NEW values]
)

#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Estimations Based on Corine 2012 
### Reclassify the raster so that it ONLY shows the areas considered AGRICULTURAL AREAS 
CorineDK.2012.Past <- reclassify(x = CorineDK.2012, # raster to reclasify
                                 rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(CorineDK.2012.Past)
...
CorineDK.2012.Past1k <- aggregate(x = CorineDK.2012.Past, # raster to aggregate
...
CorineDK.2018.Past <- reclassify(x = CorineDK.2018, # raster to reclasify
                                 rcl = ReclassMrtx # the Reclassification matrix
)
### Now lets look at it
plot(CorineDK.2018.Past)
...
CorineDK.2018.Past1k <- aggregate(x = CorineDK.2018.Past, # raster to aggregate
                                  fact = 10, # Aggregation factor (number of cells in each direction to merge together)
                                  fun = Summ.Func) # how to aggregate the cells
### Now lets look at this reclasified map
plot(CorineDK.2018.Past1k)

#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Average of Open Natural density in 2012 and 2018
CorineDK.Past1k <- mean(stack(CorineDK.2018.Past1k,CorineDK.2012.Past1k))

plot(CorineDK.Past1k)

## Save the Raster
writeRaster(x = CorineDK.Past1k, # raster to save
            filename = "/Users/alejandroordonez/Dropbox/Courses_&_Conferences/2022/Courses/Statistical and Geospatial Modeling/Lectures-Practical/Project/Data/Agro Type Density/PastDens_Cor.tif", # Name of the raster
            format="GTiff", overwrite=TRUE) # Arguments
