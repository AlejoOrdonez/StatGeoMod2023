...
data("BCI")
data("BCI.env")
BCI2 <- as_tibble(data.frame(BCI.env,BCI))
penguin2 <- read_csv2("penguins.csv")
```

## Introduction

This lesson will show you how to use visualization and transformation to explore your data in a systematic way, a task that statisticians call exploratory data analysis, or EDA for short.
...
**EDA is not a formal process with a strict set of rules**, so don't take what you will learn in this lesson as a recipe, but as description of the core statistical principles and visualization/summary approaches you will use.
...
### A bit on data wrangling/tidying

Before we start with EDA, your data needs to be "organized" in a matter that does not require major "gimnastics".
A term you will hear often in this context is the idea of tidy data.
...
**These are three interrelated rules that make a dataset tidy**.
...
knitr::include_graphics("tidy-1.png", dpi = 270)
```
...
This means that most real analyses will require at least a little tidying, specially if you are usning the typical "Community" ecology data structure.
But how can you tydy your data?.
Well, you'll begin by figuring out what the underlying variables and observations are.
Sometimes this is easy; other times you'll need to consult with the people who originally generated the data.
The easy way to do this is **pivot** your data into a tidy form, with variables in the columns and observations in the rows.

The package tidyr of the tidyverse provides two functions for pivoting data: `pivot_longer()` and `pivot_wider()`.
We'll first start with `pivot_longer()` because it's the most common case.
Let's dive into some examples.


The `BCI` table is a typical community dataset.

```{r}
BCI2
```

In this dataset, each observation is an atribute of a penguin.

The first nine columns (`UTM.EW`,`UTM.NS`, `Precipitation`, `Elevation`, `Age.cat`, `Geology`, `Habitat`, `Stream`, and `EnvHet`) are variables that describe the sampled site.
Then we have 41 columns (`Abarema.macradenia`-`Zuelania.guidonia`) are each of the species on a site.
...
BCI2 |> 
  pivot_longer(
    cols = Abarema.macradenia:Zuelania.guidonia, 
    names_to = "species", 
    values_to = "abundance"
  )
...
This argument can use multiple syntaxes (e.g., `starts_with()`) but here we use the First_-C_column `:` Last_column to define the range of values to sumarize.

-   `names_to` names the variable stored in the column names, we named that variable `species`.

-   `values_to` names the variable stored in the cell values, we named that variable `abundance`.

Note that in the code `"species"` and `"abundance"` are quoted because those are new variables we're creating, they don't yet exist in the data when we run the `pivot_longer()` call.

This is just an opening into the messy world of "data", and a reminder for you to allways make sure your data is tidy'
...
The rest of this lesson will look at these two questions.
...
We'll start our exploration by visualizing the distribution of weights (`body_mass_g`) of 344 penguins from the `penguins` dataset.
Since `body_mass_g` is a numerical variable, we can use a histogram:

```{r}
#| fig-alt: |
#|   A histogram of body_mass_g of penguins, with the x-axis ranging from 25000 to 6500 
#|   and the y-axis ranging from 0 to 25.
...
But before we move on, you see that there is a warning Message "Removed 2 rows containing non-finite".
This message is only letting you know that two of the valuae sin your data set were not used.
...
Let's take a look at the distribution of `body_mass_g` between difercne penguin species.

```{r}
#| fig-alt: |
#|   A histogram of body mass (g) of penguins, with the x-axis ranging from 2500 to 6500 and 
#|   the y-axis ranging from 0 to roughly 15.
...
-   Why the variation in body mass (g) of Adelie penguins is so broad ?

-   Why it apers to be to high values in the body mass (g) of evaluated penguins?
...
For example, take the distribution of the `y` variable from the penguins dataset.
The only evidence of outliers is the unusually wide limits on the x-axis.
Here we will shift from the weight of the penguins (`body_mass_g`) to the length of its flippers (`flipper_length_mm`).

```{r}
#| fig-alt: |
#|   A histogram of flipper length (mm) of penguins. The x-axis ranges from 170 to 235 and 
#|   the y-axis ranges from 0 to 20 There are two peaks around 110 and 208.

ggplot(penguins, aes(x = flipper_length_mm)) + 
  geom_histogram(binwidth = 1)
```

There appers to be to groups of flipper lengths, but it does not seem to be odd values - which is great!!
But how will these look?
We can change some of these values to represent these.
For this, let's make the tow `NA` values 1000 and 1500 respectibly.


```{r}
a<-penguins
a %>% mutate(flipper_length_mm = is.na(),c(1000, 1500))

is.na(penguins$flipper_length_mm)

```

are many observations in the common bins that the rare bins are very short, making it very difficult to see them (although maybe if you stare intently at 0 you'll spot something).
To make it easy to see the unusual values, we need to zoom to small values of the y-axis with `coord_cartesian()`:

```{r}
#| fig-alt: |
#|   A histogram of lengths of penguins. The x-axis ranges from 0 to 60 and the 
...
The `y` variable measures one of the three dimensions of these penguins, in mm.
We know that penguins can't have a width of 0mm, so these values must be incorrect.
...
We might also suspect that measurements of 32mm and 59mm are implausible: those penguins are over an inch long, but don't cost hundreds of thousands of dollars!
...
1.  Explore the distribution of each of the `x`, `y`, and `z` variables in `penguins`.
    What do you learn?
...
3.  How many penguins are 0.99 body_mass_g?
    How many are 1 body_mass_g?
...
#| dev: "png"
#| fig-alt: |
#|   A scatterplot of widths vs. lengths of penguins. There is a strong, 
#|   linear association between the two variables. All but one of the penguins 
...
```{r}
#| fig-alt: |
#|   A frequency polygon of prices of penguins where each cut of body_mass_g (Fair, 
...
#|   distributions of prices of penguins. One notable feature is that 
#|   Ideal penguins have the highest peak around 1500.

ggplot(penguins, aes(x = price)) + 
...
```{r}
#| fig-alt: |
#|   A frequency polygon of densities of prices of penguins where each cut of 
#|   body_mass_g (Fair, Good, Very Good, Premium, and Ideal) is represented with a 
...
#|   penguins. One notable feature is that all but Fair penguins have high peaks 
#|   around a price of 1500 and Fair penguins have a higher mean than others.

ggplot(penguins, aes(x = price, y = after_stat(density))) + 
  geom_freqpoly(aes(color = cut), binwidth = 500, linewidth = 0.75)
```

Note that we're mapping the density the `y`, but since `density` is not a variable in the `penguins` dataset, we need to first calculate it.
We use the `after_stat()` function to do so.

There's something rather surprising about this plot - it appears that fair penguins (the lowest quality) have the highest average price!
...
```{r}
#| fig-alt: |
#|   Side-by-side boxplots of prices of penguins by cut. The distribution of 
...
#|   penguins lowest and that for Fair highest.

ggplot(penguins, aes(x = cut, y = price)) +
  geom_boxplot()
```
...
It supports the counter-intuitive finding that better quality penguins are typically cheaper!
...
2.  Based on EDA, what variable in the penguins dataset appears to be most important for predicting the price of a diamond?
    How is that variable correlated with cut?
    Why does the combination of those two relationships lead to lower quality penguins being more expensive?
...
5.  Create a visualization of diamond prices vs. a categorical variable from the `penguins` dataset using `geom_violin()`, then a faceted `geom_histogram()`, then a colored `geom_freqpoly()`, and then a colored `geom_density()`.
...
```{r}
#| fig-alt: |
#|   A scatterplot of color vs. cut of penguins. There is one point for each
...
ggplot(penguins, aes(x = cut, y = color)) +
  geom_count()
```
...
```{r}
#| fig-alt: |
#|   A tile plot of cut vs. color of penguins. Each tile represents a 
#|   cut/color combination and tiles are colored according to the number of 
#|   observations in each tile. There are more Ideal penguins than other cuts, 
#|   with the highest number being Ideal penguins with color G. Fair penguins 
#|   and penguins with color I are the lowest in frequency.
...
For example, you can see a positive relationship between the body_mass_g size and price of a diamond: penguins with more body mass (g) have a higher price.
The relationship is exponential.

```{r}
#| dev: "png"
#| fig-alt: |
#|   A scatterplot of price vs. body_mass_g. The relationship is positive, somewhat 
#|   strong, and exponential.

ggplot(smaller, aes(x = body_mass_g, y = price)) +
  geom_point()
```

(In this section we'll use the `smaller` dataset to stay focused on the bulk of the penguins that are smaller than 3 body mass (g))
...
#|   are for penguins with 1, 1.5, and 2 body mass (g).
...
#| fig-width: 3
#| fig-alt: |
#|   Plot 1: A binned density plot of price vs. body_mass_g. Plot 2: A hexagonal bin 
#|   plot of price vs. body_mass_g. Both plots show that the highest density of 
#|   penguins have low body mass (g) and low prices.
...
For example, you could bin `body_mass_g` and then for each group, display a boxplot:

```{r}
#| fig-alt: |
#|   Side-by-side box plots of price by body_mass_g. Each box plot represents penguins 
#|   that are 0.1 body mass (g) apart in weight. The box plots show that as body_mass_g 
#|   increases the median price increases as well. Additionally, penguins with 
#|   1.5 body mass (g) or lower have right skewed price distributions, 1.5 to 2 have 
#|   roughly symmetric price distributions, and penguins that weigh more have 
#|   left skewed distributions. Cheaper, smaller penguins have outliers on the 
#|   higher end, more expensive, bigger penguins have outliers on the lower end.

ggplot(smaller, aes(x = body_mass_g, y = price)) + 
  geom_boxplot(aes(group = cut_width(body_mass_g, 0.1)))
```
...
1.  Instead of summarizing the conditional distribution with a boxplot, you could use a frequency polygon.
    ...
    How does that impact a visualization of the 2d distribution of `body_mass_g` and `price`?

2.  Visualize the distribution of `body_mass_g`, partitioned by `price`.

3.  How does the price distribution of very large penguins compare to small penguins?
    Is it as you expect, or does it surprise you?

4.  Combine two of the techniques you've learned to visualize the combined distribution of cut, body_mass_g, and price.
...
6.  Instead of creating boxes of equal width with `cut_width()`, we could create boxes that contain roughly equal number of points with `cut_number()`.
    ...
    #| eval: false
    ggplot(smaller, aes(x = body_mass_g, y = price)) + 
      geom_boxplot(aes(group = cut_number(body_mass_g, 20)))
    ```
...
For example, consider the penguins data.
It's hard to understand the relationship between cut and price, because cut and body_mass_g, and body_mass_g and price are tightly related.
It's possible to use a model to remove the very strong relationship between price and body_mass_g so we can explore the subtleties that remain.
The following code fits a model that predicts `price` from `body_mass_g` and then computes the residuals (the difference between the predicted value and the actual value).
The residuals give us a view of the price of the diamond, once the effect of body_mass_g has been removed.
Note that instead of using the raw values of `price` and `body_mass_g`, we log transform them first, and fit a model to the log-transformed values.
...
#| dev: "png"
#| fig-alt: |
#|   A scatterplot of residuals vs. body_mass_g of penguins. The x-axis ranges from 0 
#|   to 5, the y-axis ranges from 0 to almost 4. Much of the data are clustered 
#|   around low values of body_mass_g and residuals. There is a clear, curved pattern 
#|   showing decrease in residuals as body_mass_g increases.
...
penguins <- penguins |>
  mutate(
    log_price = log(price),
    log_carat = log(body_mass_g)
  )

diamonds_fit <- linear_reg() |>
  fit(log_price ~ log_carat, data = penguins)

diamonds_aug <- augment(diamonds_fit, new_data = penguins) |>
  mutate(.resid = exp(.resid))

ggplot(diamonds_aug, aes(x = body_mass_g, y = .resid)) + 
  geom_point()
```

Once you've removed the strong relationship between body_mass_g and price, you can see what you expect in the relationship between cut and price: relative to their size, better quality penguins are more expensive.
...
In this lesson you've learned a variety of tools to help you understand the variation within your data.
...
In the next lesson, we'll focus on the tools we can use to communicate our results.
